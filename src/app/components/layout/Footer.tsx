  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-8 pt-16 md:pt-20 pb-12">
        ...
        {/* ── Brand block — visible on mobile/tablet only, hidden on desktop (shown in col-1 grid) ── */}
        ...
        {/* ── Main grid ── */}
        {/* Mobile: 2-col grid (Products+Company row1, Contact+CTA row2) */}
        {/* Tablet (sm): 2-col grid (same) */}
        {/* Desktop (lg): 4-col grid [brand | products | company | contact+cta] */}
        <div className="grid grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1fr] gap-x-5 gap-y-8 sm:gap-8 lg:gap-8 xl:gap-12">

          {/* ── COL 1: Brand Overview — desktop only ── */}
          ...
          {/* ── COL 2: Products ── */}
          <div>
            <h4 className="text-[13px] sm:text-[15px] lg:text-base uppercase tracking-[1px] text-[#D4A017]/80 font-semibold mb-4 sm:mb-5">
              Products
            </h4>
              ...
              {products.map((p) => {
                ...
                return (
                  <li key={p.id}>
                    <a
                      href={productHref}
                      className="group flex items-start gap-2 text-xs sm:text-sm text-white/45 hover:text-white transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#D4A017] rounded leading-snug"
                    >
                      <span
            ...
            <ul className="space-y-3 sm:space-y-3.5" role="list">
              {COMPANY_LINKS.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    className="group flex items-center gap-2 text-xs sm:text-sm text-white/45 hover:text-white transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#D4A017] rounded"
                  >
                    <span
          ...
          <div>
            ...
            <div className="space-y-2.5 mb-6">
              ...
              <a
                ...
                className="flex items-center gap-2 sm:gap-2.5 text-xs text-white/45 hover:text-[#D4A017] transition-colors duration-200 group focus:outline-none focus-visible:ring-2 focus-visible:ring-[#D4A017] rounded"
              >
                <span
                ...
                <span className="truncate min-w-0">{companyInfo.email}</span>
              </a>
              ...
              >
                ...
                <span className="truncate min-w-0">{companyInfo.phone}</span>
              </a>
              ...
              <a
                ...
                className="flex items-center gap-2 sm:gap-2.5 text-xs text-white/45 hover:text-white transition-colors duration-200 group focus:outline-none focus-visible:ring-2 focus-visible:ring-[#25D366] rounded"
                aria-label="Contact us on WhatsApp"
              >
                <span
                  className="w-6 h-6 rounded-md flex items-center justify-center flex-shrink-0 bg-[#25D366]/15 group-hover:bg-[#25D366]/25 transition-colors"
                  aria-hidden="true"
                >
            ...
            {/* Request Quote CTA — hidden on mobile (rendered in separate col below) */}
            <div
              className="hidden sm:block rounded-xl border border-white/[0.08] p-5"
              style={{
                background:
          ...
          {/* ── COL 5: Ready to Source — mobile only (row 2, col 2) ── */}
          <div className="block sm:hidden">
            <h4 className="text-[13px] uppercase tracking-[1px] text-[#D4A017]/80 font-semibold mb-4">
              Source at Scale
            </h4>
            <div
            ...
            >
              ...
              <a
                href="/#request-quote"
                data-analytics={JSON.stringify({ type: "request_quote", label: "Footer Request Bulk Quote Mobile" })}
                className="flex items-center justify-center gap-2 w-full bg-[#D4A017] text-[#0a1f16] text-xs font-bold py-2.5 rounded-lg hover:bg-[#e6b420] transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#D4A017] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0d2d1f]"
              >
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none" aria-hidden="true">
                  <path d="M1.5 6.5h9M7 3l3 3.5L7 10" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
